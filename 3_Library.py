...
# Entities/Table: Book, User, Library
...
class Library:
    ...
    def addBook(self,cat,id,name,q):
        ...
        print(f"\n\t{name} Book added ! ") #function ke call kore book add korle eta print hobe function theke.
        ...
        for book in self.books:
            if book.id in id:
                    if book in user.borrowedBooks:
                     ...
                     book.quan += 1
                     print(f'\n\t{book.name} Return the book successfully')
                     return
...
while run:
    ...
    if currentUser == None: #currentUser None hole user regi or login er jonno offer krbo
        print(f"\n\tNo logged in user ! ")
